...
// The body and feet share one gait clock. Contacts are world-space anchors;
// only the swing foot travels, and every walk ends with a planted stance.
export class Footwork {
  constructor(THREE,actor,bones,hipPosition,modelHeight){
    ...
    this.state={stepWave:0,pelvisDrop:.008*this.scale,lateral:0,forwardLean:0,settled:true,movingWeight:0,supportSide:null,swinging:false};
    this.seed();
  }
  seed(){
    const T=this.THREE;
  ...
  advance(delta,{velocity,turnRate=0,moving=false,stopping=false,reducedMotion=false}={}){
    ...
    if(this.swing){
      ...
      const toeOff=.17*Math.sin(Math.PI*clamp(u/.55,0,1));
      const heel=-.09*ease((u-.55)/.45);
      foot.pitch=swing.fromPitch*(1-ease(u/.22))+toeOff+heel;
      this.state.stepWave=(swing.index===0?-1:1)*Math.cos(Math.PI*u);
      if(u===1){foot.flat.copy(swing.to);foot.heading.copy(swing.toHeading);foot.landingPitch=foot.pitch;foot.stanceAge=0;this.swing=null;this.doubleSupport=travelling?.08:.065;}
    }

    if(!this.swing&&this.doubleSupport===0){
      const duration=travelling?.405-.045*clamp(speed/(.55*s),0,1):turning?.29:.31;
      const lead=travelling?duration+(duration+.08)*.45:0;
      const candidates=this.feet.map((foot,index)=>{
        const neutral=this.neutral(foot),to=neutral.clone().addScaledVector(vel,lead);
        const offset=to.clone().sub(neutral);if(offset.length()>.175*this.modelHeight)to.copy(neutral).add(offset.setLength(.175*this.modelHeight));
      ...
      const needsStep=travelling?(next.error>.035*s||next.angle>.12):(next.error>.017*s||next.angle>.11);
      if(needsStep){
        this.startStep(next.index,next.to,heading,reducedMotion?.10:duration,reducedMotion?.004:.024*this.modelHeight*(travelling?1:.65));
      }
    }
    ...
    const lateralTarget=support?clamp(support.rest.x*.19,-.012*s,.012*s):0;
    ...
    for(const foot of this.feet){
      ...
      const reach=foot.ik.reach-.002*s,vertical=Math.sqrt(Math.max(.01,reach*reach-dx*dx-dz*dz));
      drop=Math.max(drop,hip.y-foot.position.y-vertical+.002*s);
    }
    drop=clamp(drop,.007*s,.055*s);
    ...
    this.state.forwardLean=T.MathUtils.damp(this.state.forwardLean,clamp(speed*.038+acceleration*.01,-.025,.04),8,dt);
